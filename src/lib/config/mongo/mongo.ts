...
export class Mongo {
  ...
  private constructor(private mongoConfig: MongoConfig) {}

  // Static method to get or create the Mongo instance
  public static getInstance(mongoConfig?: MongoConfig): Mongo {
    if (!instance && mongoConfig) {
      instance = new Mongo(mongoConfig); // Create new instance if it doesn't exist
    }
    return instance as Mongo;
  }
  ...
  private async mongoConnectionFactory(): Promise<void> {
    try {
      ...
      log.debug("Initializing MongoDB connection", {
  ...
  async disconnectDB(): Promise<void> {
    if (!this.isConnected) {
      log.info("MongoDB is already disconnected.");
